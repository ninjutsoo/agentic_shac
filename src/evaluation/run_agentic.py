"""
Agentic Pipeline runner for Drug StatusTime classification (Phase 3).

Loads processed JSONL data, runs the agentic pipeline, saves predictions,
...
def main():
    parser = argparse.ArgumentParser(description="Run agentic pipeline inference and evaluation")
    parser.add_argument("--split", nargs="+", default=["dev"], help="Splits to evaluate (e.g., dev test)")
    parser.add_argument("--config", default="configs/agentic.yaml", help="Path to agentic config YAML")
    parser.add_argument("--data_dir", default="data/processed", help="Directory with processed JSONL files")
    parser.add_argument("--out_dir", default="experiments/agentic", help="Output directory for predictions")
    ...
    print("\n" + "=" * 80)
    print("✅ Agentic Pipeline Runner Complete")
    print("=" * 80)
